...
@register_model("w2vAutoEncoder")
class Wav2VecAutoEncoderModel(BaseFairseqModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            "--conv-feature-layers_en",
            type=str,
            metavar="EXPR",
            help="convolutional feature extraction layers [(dim, kernel_size, stride), ...]",
        )
        parser.add_argument(
            "--conv-aggregator-layers_en",
            type=str,
            metavar="EXPR",
            help="convolutional feature extraction layers [(dim, kernel_size, stride), ...]",
        )
    ...
    def build_model(cls, args, task):
        ...
        model = Wav2VecAutoEncoderModel(args)
        logger.info(model)
        return model
...
#  add encoder for Auto-encoder
        if args.encoder == "cnn":
            feature_enc_layers = eval(args.conv_feature_layers_en)
            self.feature_extractor_en = ConvFeatureExtractionModel_en(
                conv_layers=feature_enc_layers,
                dropout=0.0,
                log_compression=args.log_compression,
                skip_connections=args.skip_connections_feat,
        ...
        def make_aggregator_en():
            if args.aggregator == "cnn":
                agg_layers = eval(args.conv_aggregator_layers_en)
                agg_dim = agg_layers[-1][0]
                feature_aggregator = ConvAggegator_en(
                    conv_layers=agg_layers,
                    embed=embed,
                    dropout=args.dropout,
                    skip_connections=args.skip_connections_agg,
        ...
        self.feature_aggregator_en, agg_dim = make_aggregator_en()
        ...
        self.dropout_feats_en = nn.Dropout(p=args.dropout_features)
        self.dropout_agg_en = nn.Dropout(p=args.dropout_agg)
    ...
    def forward(self, source):
        result = {}
        # print('|****source_S|', source.shape)
        # source = source.view(source.shape[0], -1, 512).transpose(1, 2) # feature dim 
        # print('|****viewed_source_S|', source.shape)
# ######## AutoEncoder 架构

        # -------- Encoder, Transmitter ---------- 
        # source = source + 1   # upraise the audio energy
        T_a = self.feature_extractor_en(source)
        T_a = self.dropout_feats_en(T_a)
        X = self.feature_aggregator_en(T_a)
        X = self.dropout_agg_en(X)
        # print("|*******X_S:|", X.shape)
        ...
        P_noise = torch.tensor(P_noise).half().cuda()
        # [r1,r2,r3] = X.shape 
        # noise = np.random.randn(r1,r2,r3)
        noise = torch.randn(X.shape).half().cuda()
        # print(X.type(), torch.from_numpy(noise).float().cuda().type(), P_noise.type())
        # Y = X + torch.from_numpy(noise).half().cuda() * P_noise.cuda()
        Y = 0.8 * X.cuda() + noise.cuda() * P_noise.cuda()

        # --------- Decoder, Receiver ------------
        # source = source.view(source.shape[0], -1, 512).transpose(1, 2)
        features = self.feature_aggregator(Y)
        ...
        features_ = self.dropout_agg(features)
        x = self.feature_extractor(features_)
        # print("|in decoder model, out feature_extractor|", x.shape)
        x_ = self.dropout_feats(x)
        # x_= x_ - 1        # decrease the audio energy bias
        result["out_audio"] = x_
        # result[""]


        # features = self.feature_extractor(source)
        # print('\n|***source_S***|', source.shape, '\n')
        # if self.vector_quantizer:
        #     q_res = self.vector_quantizer(features)
        #     features = q_res["x"]
        #     for k in q_res.keys():
    ...
    def max_positions(self):
        ...
        return sys.maxsize
    def get_out_audio(self, net_output):
        out_audio = net_output["out_audio"]
        return out_audio
...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
        ...
        def block(n_in, n_out, k, stride, is_last_layer):
            activation = nn.ReLU() if not is_last_layer else nn.Tanh()
            if is_last_layer:
                return nn.Sequential(
                    nn.ConvTranspose1d(n_in, n_out, k, stride=stride, bias=False),
        ...
        in_d = conv_layers[0][0] # in_dim of embedding dim
        self.conv_layers = nn.ModuleList()
        for index, [dim, k, stride] in enumerate(conv_layers):
            is_last_layer = False
            if index == len(conv_layers) - 1:
                is_last_layer = True
            self.conv_layers.append(block(in_d, dim, k, stride, is_last_layer))
            in_d = dim
    ...
    def forward(self, x):
        # BxT -> BxCxT
        # print('x_Shape_in_extractor:', x.shape)
        # x = x.unsqueeze(1)
        ...
        if self.log_compression:
            ...
            x = x + 1
            x = x.log()
        # print('x_Shape_out_extractor:', x.shape)
        return x
...
class ConvAggegator(nn.Module):
    def __init__(
        ...
        def block(n_in, n_out, k, stride):
            ...
            pad = (
                ZeroPad1d(ka + kb, 0) if zero_pad else nn.ReplicationPad1d((k+1, 0))
            )

            return nn.Sequential(
                pad,
                nn.ConvTranspose1d(n_in, n_out, k, stride=stride, bias=conv_bias, padding = k),
                nn.Dropout(p=dropout),
                norm_block(False, n_out, affine=not non_affine_group_norm),
                # activation,
            )

        # in_d = embed   # origninally in_d
        in_d = conv_layers[0][0]
        self.conv_layers = nn.ModuleList()
        self.residual_proj = nn.ModuleList()
        for dim, k, stride in conv_layers:
            if in_d != dim and skip_connections:
                self.residual_proj.append(nn.ConvTranspose1d(in_d, dim, 1, bias=False))
            else:
    ...
    def forward(self, x):
        for rproj, conv in zip(self.residual_proj, self.conv_layers):
            ...
            if self.skip_connections:
                ...
                # print('x_S:', x.shape)
                # print('residualS:', residual.shape)
                # print('residual_scale_S:', self.residual_scale)
                x = (x + residual) * self.residual_scale
        act_tanh = nn.Tanh()
        x = act_tanh(x)
        return x
...
class Wav2VecPredictionsModel(nn.Module):
    def __init__(
        ...
        self.project_to_steps = nn.ConvTranspose2d( # originally Trans2d
    ...
    def forward(self, x, y):
        ...
        print('****|x.shape|', x.shape, self.offset)
        print('****|orign_targets.shape|', targets.shape)
        ...
        print('|****in_model_pred:|', predictions.shape)
        if self.infonce:
...
###### add Auto-encoder‘s dual architecture, based on w2vdecoder's arch, so the followings are encoder's part

class ConvFeatureExtractionModel_en(nn.Module):
    def __init__(
        self,
        conv_layers,
        dropout,
...
class ConvAggegator_en(nn.Module):
    def __init__(
        self,
        conv_layers,
        embed,
...
@register_model_architecture("w2vAutoEncoder", "w2vAutoEncoder")
def base_wav2vec_architecture(args):
    ...
    args.conv_feature_layers_en = getattr(args, "conv_feature_layers_en", conv_feature_layers)
    args.conv_aggregator_layers_en = getattr(args, "conv_aggregator_layers_en", "[(512, 3, 1)] * 9"    )
    ...
    args.dropout_features = getattr(args, "dropout_features", 0)  # add to 0.1 by thn on Jar 31st 2021
    args.dropout_agg = getattr(args, "dropout_agg", 0)            # add to 0.1 by thn on Jar 31st 2021
    ...
    args.log_compression = getattr(args, "log_compression", False)   # change in CLI in 0201 by thn
    ...
    args.activation = getattr(args, "activation", "tanh")           # change activate func
